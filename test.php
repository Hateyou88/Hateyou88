...
function getItemInfo ($productLink) {
    try {
        ...
        $options = ($dom->find('.tabs-panel .option')->count() && $dom->find('.tabs-panel .option')) ? $dom->find('.tabs-panel .option') : []; //Остальные данные: размер, вес, характеристики и бренд
        ...
        $stock = ($quantityNode->count() && (int)$quantityNode->getAttribute('max')) ? 'In stock' : 'Out of stock'; // Доступность товара, если есть на складе хотя бы одна единица товара ,то выводим её
